...
static const char* PAPER_ID = "ssrn-4962098";
static const std::string TITLE = R"MW4LLM()MW4LLM";
static const std::string SSRN_URL = R"MW4LLM(https://papers.ssrn.com/sol3/papers.cfm?abstract_id=4962098)MW4LLM";
...
static const std::string SUMMARY_MD = R"MW4LLM(Here is the bullet list for "The Readability of Contracts: A Big Data Analysis" (ssrn-4962098) by Professor Yonathan Arbel:
...
*   ## 各章节摘要
    ...
    *   **讨论、影响与结论：** 阿拉巴马大学法学院的约纳森·阿尔贝教授写道，普遍存在的合同不可读性增加了消费者的交易成本，造成了对消费者不利的信息不对称，并从根本上削弱了知情同意的理想状态，表明低可读性可能服务于起草方的策略性目的，或源于路径依赖等系统性问题。他指出，提高合同可读性是一项关键的政策目标，可能通过改进起草实践、人工智能驱动的摘要等技术辅助手段，或设定可读性标准的监管干预来实现。这种“理解鸿沟”要求对法律原则进行重新评估，并在合同法中引入一种新的、基于实证的现实主义。)MW4LLM";
static const std::string ONE_PAGER_MD = R"MW4LLM(#  — one-page summary
...
static const std::string STUDY_PACK_MD = R"MW4LLM(# Study pack:  (ssrn-4962098)
...
static const std::string ARTICLE_TEXT = R"MW4LLM(
...
ARBEL, THE READABILITY OF CONTRACTS: BIG DATA ANALYSIS 55/55)MW4LLM";
