...
static const char* PAPER_ID = "ssrn-3519630";
static const std::string TITLE = R"MW4LLM(ALL-CAPS)MW4LLM";
static const std::string SSRN_URL = R"MW4LLM(https://papers.ssrn.com/sol3/papers.cfm?abstract_id=3519630)MW4LLM";
...
static const std::string SUMMARY_MD = R"MW4LLM(Bullet list for 'ssrn-3519630' by Professor Yonathan Arbel of the University of Alabama School of Law:
...
2.  ## 各章节摘要（每条120字以内）
    ...
    *   **结论与行动呼吁：摒弃全大写并创新披露方式**
        阿拉巴马大学法学院的Yonathan Arbel教授在其研究中指出，认为全大写能提升同意程度的信念是一种迷信，缺乏实证支持且损害老年读者。法院应摒弃将全大写作为同意的代表的做法，因为它扼杀了披露方式的创新。他建议采用一种基于绩效的方法，即公司必须证实条款已做到醒目，从而引导其专业能力转向设计真正易于理解的披露。未来的改进取决于此类标准以及对异质性效应的考量，因为当前如全大写等做法对弱势群体造成了不成比例的损害。粗体文本等替代方案的成功为设计更好的披露方式带来了乐观前景。)MW4LLM";
static const std::string ONE_PAGER_MD = R"MW4LLM(# ALL-CAPS — one-page summary
...
static const std::string STUDY_PACK_MD = R"MW4LLM(# Study pack: ALL-CAPS (ssrn-3519630)
...
static const std::string ARTICLE_TEXT = R"MW4LLM(ALL-CAPS
ALL-CAPS
...
Electronic copy available at: https://ssrn.com/abstract=3519630)MW4LLM";
