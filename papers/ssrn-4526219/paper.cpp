...
static const char* PAPER_ID = "ssrn-4526219";
static const std::string TITLE = R"MW4LLM(Public Law and Legal Theory Research Paper Series)MW4LLM";
static const std::string SSRN_URL = R"MW4LLM(https://papers.ssrn.com/sol3/papers.cfm?abstract_id=4526219)MW4LLM";
...
static const std::string SUMMARY_MD = R"MW4LLM(Here's the requested information for 'ssrn-4526219' by Professor Yonathan Arbel:
...
2.  ## 各章节摘要（每节120词以内）
    ...
    *   阿拉巴马大学法学院的约纳坦·阿尔伯教授指出，在审慎使用和保持透明度的前提下，生成式解释提供了一种易于获取且可预测的工具，它重新定义了合同解释的争议。它既可以作为一种更精确的文本主义方法，也可以作为一种更高效的语境主义方法，能够处理大量证据并评估其证明价值。这种新方法有望带来可预测性、语言准确性并降低成本，可能改变默认规则以更包容外部证据，并纠正精英主义倾向。他认为这提供了一个重要的中间地带，可能成为主流的默认方法。最终，他断言其未来具有高度颠覆性，可能降低正式合同的价值。)MW4LLM";
static const std::string ONE_PAGER_MD = R"MW4LLM(# Public Law and Legal Theory Research Paper Series — one-page summary
...
static const std::string STUDY_PACK_MD = R"MW4LLM(# Study pack: Public Law and Legal Theory Research Paper Series (ssrn-4526219)
...
static const std::string ARTICLE_TEXT = R"MW4LLM(Public Law and Legal Theory Research Paper Series
